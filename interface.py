...
class TerereApp:
    ...
    def aplicar_tema(self):
        ...
        self.style.configure("TRadiobutton", background=t["bg"], foreground=t["fg"], font=("Helvetica", 10))
        self.style.map("TRadiobutton", background=[("active", t["bg"])], foreground=[("active", t["accent"])])
    ...
    def construir_aba_quiz(self):
        ...
        self.lbl_quiz_palavra.pack(pady=(20, 10))
        ...
        for i in range(4):
            btn = tk.Button(
                ...
                relief="flat", 
                borderwidth=1,
                command=lambda idx=i: self.checar_resposta_quiz(idx)
            )
            btn.pack(pady=6)
            self.botoes_quiz.append(btn)
            
        self.lbl_quiz_score = ttk.Label(self.aba_quiz, font=("Helvetica", 12, "bold"))
        self.lbl_quiz_score.pack(pady=(25, 15))
        ttk.Button(self.aba_quiz, textvariable=self.vars_textos["btn_next"], cursor="hand2", command=self.gerar_pergunta_quiz).pack(pady=10)
        self.gerar_pergunta_quiz()
